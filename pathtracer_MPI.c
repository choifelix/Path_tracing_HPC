...
void version3_simd(int argc, char **argv){
	MPI_Init(&argc,&argv);
	int rank,size;
	MPI_Comm_size(MPI_COMM_WORLD,&size);
	MPI_Comm_rank(MPI_COMM_WORLD,&rank);
...
void version4_openMP(int argc, char **argv){
	MPI_Init(&argc,&argv);
	int rank,size;
	MPI_Comm_size(MPI_COMM_WORLD,&size);
	MPI_Comm_rank(MPI_COMM_WORLD,&rank);
	...
	if (rank == 0){
		...
		sprintf(nom_rep, "/nfs/home/sasl/eleves/main/3520621/Documents/HPC/Path_tracing_HPC/%s", pass->pw_name);
		mkdir(nom_rep, S_IRWXU);
		sprintf(nom_sortie, "%s/version4_image%d.ppm", nom_rep,size);
...
void traitement_token_omp(int rank, int size,int token, bool work, int *state, bool *continuer, int *i, int work_limit){
	//printf("proc %d entering token managment \n",rank);
	int *token_send;
	if(token  == -2){
		//printf("proc %d  case : -2\n",rank);
...
void version5_openMP_com(int argc, char **argv){
	MPI_Init(&argc,&argv);
	int rank,size;
	MPI_Comm_size(MPI_COMM_WORLD,&size);
	MPI_Comm_rank(MPI_COMM_WORLD,&rank);
			...
			while(continuer){
				...
				if(!work){
					...
					if(token == -2){
						...
						//printf("proc %d  token here , checking\n",rank);
						traitement_token_omp(rank, size, token, work, &state, &continuer, &i, work_limit);
						token = -10;
					}
				}
